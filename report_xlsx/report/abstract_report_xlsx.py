...
class AbstractReportXslx(models.AbstractModel):
    _name = 'report.abstract_report_xlsx'
    ...
    def write_array_sub_filter_columns_header(self):
        """Write array header on current line using all defined columns name.
        Columns are defined with `_get_report_sub_filter_columns` method.
        """
        for col_pos, column in self.sub_filter_columns.items():
            self.sheet.write(self.row_pos, col_pos + 1, column['sub_filter_header'],
                             self.format_header_center)
        self.row_pos += 1

    def write_sub_filter_line(self, line_object):
        ...
        for col_pos, column in self.sub_filter_columns.items():
            ...
            if cell_type == 'many2one':
                ...
                if line_object.currency_id:
                    ...
                    self.sheet.write_number(
                        self.row_pos, col_pos + 1, float(value), format_amt
                    )
    ...
    def write_line(self, line_object):
        ...
        for col_pos, column in self.columns.items():
            value = getattr(line_object, column['field'])
            cell_type = column.get('type', 'string')
            cell_align = column.get('align')
            if cell_type == 'many2one':
                self.sheet.write_string(
                    self.row_pos, col_pos, value.sudo().name or '', self.format_left
                    if cell_align == 'left' else self.format_right)
            elif cell_type == 'string':
                try:
    ...
    def _get_report_sub_filter_columns(self, report):
        """
            :return: the report sub filters as list

            :Example:

            [
                ['first_subfilter_name', 'first_subfilter_value'],
                ['second_subfilter_name', 'second_subfilter_value']
            ]
        """
        return False
